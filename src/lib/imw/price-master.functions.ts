...
export const updatePriceMaster = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ results: PriceMasterUpdateResult[] }> => {
    ...
    const { data: cfg } = await supabaseAdmin
      .from("sap_api_configs")
      .select("*")
      .eq("name", EDIT_CONFIG_NAME)
      .maybeSingle();
    if (!cfg)
      throw new Error(
        `SAP API config "${EDIT_CONFIG_NAME}" not found. Configure it in Admin → SAP API.`,
      );
    if (!cfg.is_active) throw new Error(`SAP API config "${EDIT_CONFIG_NAME}" is disabled.`);
