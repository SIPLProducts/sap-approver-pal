/**
 * MM ZNFA — F4 help for the Main NFA Number field.
 * Config: ZNFA_NFA_GET_API
 * Payload: { USER }
 * Response: [{ NFA_NO }, ...]
 */
...
const CONFIG_NAME = "ZNFA_NFA_GET_API";

export type ZnfaNfaListResponse = {
  numbers: string[];
  error: string | null;
...
function fail(error: string | null, sapMessage: string | null = null): ZnfaNfaListResponse {
...
function pickRows(payload: any): any[] {
  ...
  if (payload && typeof payload === "object") {
    const candidates = [payload.NFA_LIST, payload.ZNFA_LIST, payload.DATA, payload.data, payload.ITEMS];
...
export const fetchZnfaNfaList = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) =>
    z.object({ user: z.string().trim().min(1, "SAP user id is required").max(60) }).parse(d),
  )
  .handler(async ({ data }): Promise<ZnfaNfaListResponse> => {
    ...
    const inputs: Record<string, any> = { USER: data.user.trim() };
    ...
    try {
      ...
      await supabaseAdmin.from("sap_api_sync_log").insert({
        ...
        message: `znfa-nfa-list network: ${errMsg}`,
      });
    ...
    if (!res.ok) {
      await supabaseAdmin.from("sap_api_sync_log").insert({
        ...
        status: "error",
        latency_ms,
        message: `znfa-nfa-list: ${res.status} ${text.slice(0, 500)}`,
      });
    ...
    if (!Array.isArray(sapJson) && status === "FALSE") {
    ...
    for (const row of rows) {
      const raw =
        typeof row === "string" || typeof row === "number"
          ? String(row)
          : String(row?.NFA_NO ?? row?.ZNFA_NUM ?? row?.NFA_NUM ?? "");
      const v = raw.trim();
    ...
    await supabaseAdmin.from("sap_api_sync_log").insert({
      ...
      message: `znfa-nfa-list: ${res.status} ${res.statusText}`,
    });
