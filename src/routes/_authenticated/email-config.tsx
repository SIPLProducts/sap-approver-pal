...
import {
  ...
  saveNoReplyEmailConfig,
  sendNoReplyTestEmail,
} from "@/lib/admin/email-config.functions";

export const Route = createFileRoute("/_authenticated/email-config")({
  head: () => ({
    meta: [
      { title: "Email Configuration — Resustainability Approvals" },
      { name: "description", content: "Manage the No-Reply SMTP sender for outbound emails." },
      { property: "og:title", content: "Email Configuration" },
      { property: "og:description", content: "Manage the No-Reply SMTP sender for outbound emails." },
    ],
  }),
...
function EmailConfigPage() {
  ...
  const [encryption, setEncryption] = useState<"none" | "ssl" | "tls" | "starttls">("tls");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        toast.error("Failed to load email configuration", { description: (e as Error).message });
      } finally {
  ...
  async function onTest() {
    ...
    try {
      ...
      toast.error("Test email failed", { description: (e as Error).message });
    } finally {
      setTesting(false);
    }
  }
      ...
      <div className="bg-card border border-border rounded-lg shadow-card p-6 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            System sender used for vendor-related notifications (invitations, submission alerts, password resets, etc.).
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={encryption} onValueChange={(v) => setEncryption(v as typeof encryption)}>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="username" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="user@example.com" />
          </div>
        ...
        <Alert className="border-info/30 bg-info/5">
          ...
          <AlertDescription className="text-sm">
            This account is the <strong>From</strong> address for buyer notifications and the SAP-triggered password reset emails.
          </AlertDescription>
        </Alert>
