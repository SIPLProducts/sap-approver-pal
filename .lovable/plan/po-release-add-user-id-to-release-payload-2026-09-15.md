# PO Release — add USER_ID to Release payload

## Goal
Pass the signed-in SAP User ID dynamically in the PO Release payload (`RELEASE.USER_ID`) without changing any other behavior.

## Changes

### 1. `src/lib/mm/po-release.functions.ts`
- In `processPoAction`, update the `RELEASE` branch so it sends:
  ```json
  {
    "RELEASE": {
      "EBELN": ebeln,
...
- In the `releaseMutation` call (`onRelease`), include `user_id: sapUserId` in the payload.
